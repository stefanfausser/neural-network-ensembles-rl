...
conf="$conf_prefix"_mlp_getStateValues-singleDecisions.conf
subdir="single-allAgents/"
sed -r "s/agents = [0-9]+/agents = $agents/g" "$conf_prefix"_mlp1.conf > $conf
actionStr="--bestActions $maxBestActions"
...
#                    echo Copy over experiment1/"$savFile""$i2"_"$t2"_0
        ...
        do
            ...
            do
                ...
                ./"$executableBinary" $environmentParametersBasic --benchmark $actionStr --retries "$retriesConsistenciesSingle" --seed "$seed2" --conf "$conf_prefix"_mlp1.conf --savfile experiment1/"$savFile""$i2"_"$t" --iterationsPerAgent 1 --calcConsistencies --evaluateStateRepo "$maxStatesSingle" --repoFile repo_it"$t2"/"$agents"/"$subdir"/"$i"/"$conf_prefix"StateRepo"$t2" --repoErrorsFile repo_it"$t2"/"$agents"/"$subdir"/"$i"/"$conf_prefix"StateRepoErrors"$i2"_"$t" --repoActionNumbersFile repo_it"$t2"/"$agents"/"$subdir"/"$i"/"$conf_prefix"StateRepoActionNumbers"$i2"_"$t" > /dev/null &
